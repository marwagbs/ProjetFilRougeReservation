...
public class ProduitDAOFileImpl  implements GenericDAO<Produit>{
	...
	public	ProduitDAOFileImpl() throws DALException{
	...
	public void LectureEtInsertionInformation(String chemin , boolean aEntete, Carte carte) throws DALException{
		DALException ce=new DALException();
		System.out.println("Verification des données");
		...
		try(Scanner scan=new Scanner(fichier)) {
			...
			while (scan.hasNext()) {
				Produit produit=lingeVersProduit(ce, scan,cpt);
	...
	private Produit lingeVersProduit(DALException ce, Scanner scan , int cpt) {
		String ligne="";
		ligne=scan.nextLine();
		Produit produit=new Produit();
		String [] attributs=ligne.split(",");
		if (attributs.length!=4) {
			ce.ajouterErreur("["+ cpt + "]"+ ligne + "(la ligne doit contenir 3 informations)");
		}else {
